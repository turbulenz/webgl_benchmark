...
RESULTS_BASE_TEMPLATE_VERSION = 2
RESULTS_BASE_TEMPLATE = {
    'version': RESULTS_BASE_TEMPLATE_VERSION,
    'config': {
        'benchmark': {
            'name': BENCHMARK_NAME_DEFAULT,
            'id': BENCHMARK_ID_DEFAULT,
            'version': BENCHMARK_VERSION,
            'type': BENCHMARK_TYPE_DEFAULT
        },
        'hardware': {},
        'browser': {},
        'playback': {},
...
def generate_results_template(results_template_name=None,
                              config_target=None,
                              hardware_name=None,
    ...
    results_template_path = normpath(path_join(TEMPLATES_DIR, results_template_filename))
    ...
    if base_template_path:
        ...
        info("Using default base template")
        base_template = RESULTS_BASE_TEMPLATE

    if base_template['version'] != 1 and base_template['version'] != 2:
    ...
    try:
        ...
        info("Adding configuration: benchmark")
        benchmark_config = results_config['benchmark']

        if base_template['version'] == 2:
            if benchmark_id:
                benchmark_config['id'] = benchmark_id

        if benchmark_name:
            benchmark_config['name'] = benchmark_name

        if benchmark_type:
            benchmark_config['type'] = benchmark_type

        if benchmark_version:
            benchmark_config['version'] = benchmark_version
    ...
    results_template_dir = dirname(results_template_path)
    mkdir(results_template_dir)
...
def main():
    ...
    benchmark_name = BENCHMARK_NAME_DEFAULT
    benchmark_id = BENCHMARK_ID_DEFAULT
    benchmark_type = BENCHMARK_TYPE_DEFAULT
    benchmark_version = BENCHMARK_VERSION

    if args.release:
        info('Release mode activated: Forcing target to static')
        args.target = 'static'


    if args.target == 'static':
        benchmark_id = 'polycraft_gl'
        benchmark_name = 'POLYCRAFT.GL'
    ...
    generate_results_template(results_template_name=results_template_name,
