...
:DEFAULT_TARGET
if exist config\default_target.txt set /p TARGET=<config\default_target.txt
...
:DEFAULT_CONFIG
if exist config\default_config.txt set /p CONFIG=<config\default_config.txt
if "%CONFIG%" == "" goto EXIT_CONFIG_MISSING else goto SET_QRES_PATH

:SET_QRES_PATH
set QRES_PATH=external\qres\bin\win32\qres.exe

if "%PYTHONPATH%" == "hj" goto SET_PYTHONPATH else goto CHECK_PYTHONPATH
...
set /p RUN_RESOLUTION=<config\default_resolution.txt
if "%RUN_RESOLUTION%" == "" goto EXIT_RESOLUTION_MISSING
%PYTHONPATH% resolution.py --parse config\default_resolution.txt > qres_run.txt
...
echo Resolution is not set. Add it to config\default_resolution.txt
...
echo or
echo      add TARGET to config\default_target.txt
...
echo      run.bat sequence_name offline
echo or
echo      add CONFIG to config\default_config.txt
goto EXIT
